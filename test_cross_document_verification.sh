#!/bin/bash
# Script to test the cross-document verification with the new prompt
...
echo -e "${YELLOW}Testing cross-document verification for Job No: ${JOB_NO}${NC}"
...
# Step 2: Extract document identifiers
echo -e "\n${YELLOW}Step 2: Extracting document identifiers...${NC}"
EXTRACT_RESPONSE=$(curl -s -X GET "http://localhost:8081/api/verification/extract-identifiers/${JOB_NO}")
echo -e "Extract Response: ${GREEN}${EXTRACT_RESPONSE}${NC}"

# Step 3: Trigger verification with cross-document checks
echo -e "\n${YELLOW}Step 3: Triggering verification with cross-document checks...${NC}"
...
echo -e "\n${YELLOW}Step 4: Waiting for verification to complete...${NC}"
MAX_ATTEMPTS=30
ATTEMPT=0
STATUS="PENDING"
...
# Step 5: Check final verification result
echo -e "\n${YELLOW}Step 5: Checking final verification result...${NC}"
...
if [ "$STATUS" == "COMPLETED" ]; then
    ...
    DISCREPANCIES=$(echo $FINAL_RESULT | grep -o '"discrepancies":\[[^]]*\]' | grep -v "[][]")
    if [ -z "$DISCREPANCIES" ]; then
        echo -e "${GREEN}No discrepancies found. All documents verified successfully!${NC}"
    else
        echo -e "${YELLOW}Discrepancies found in verification:${NC}"
        echo -e "${DISCREPANCIES}"
    fi
else
    echo -e "${RED}Verification failed with status: ${STATUS}${NC}"
fi
